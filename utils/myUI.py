...
class myHome(object):
    def __init__(self, page: flet.Page) -> None:
        ...
        self.version_dropdown=flet.Dropdown(label="Version",options=[flet.dropdown.Option(ver) for ver in ['RS', 'E', 'FrLg']],expand=1)
        self.mode_dropdown=flet.Dropdown(label="Mode",options=[flet.dropdown.Option(mode) for mode in ['Wild Encounter', 'Stationary']],expand=1)
        self.start_button=flet.FilledTonalButton(text="Start!",expand=1)
        self.direction=DirectionButton()
        self.refresh=flet.IconButton(icon=flet.icons.REFRESH,)
        self.button_row=flet.Row(controls=[self.start_button,self.refresh,self.direction],expand=1)
        ...
        self.Column1=flet.Column(controls=[self.version_dropdown,self.mode_dropdown,self.button_row],alignment=flet.alignment.center, expand=1)
        ...
        self.Column3=flet.Column(controls=[self.main_pannel],alignment=flet.alignment.center, expand=1, horizontal_alignment=flet.CrossAxisAlignment.CENTER)

        self.row=flet.Row(controls=[self.Column1,self.Column2,self.Column3],expand=1,spacing=50)
...
class MainPanel(flet.ListView):
    """panel
    """
    def __init__(self, maxlen=100, expand: int = 1, spacing: int = 10, auto_scroll: bool = True):
...
class Block(flet.Row):
    ...
    def __init__(self, text: str, another: object, expand=0) -> None:
        ...
        self.controls = [self.text, self.another, ]
